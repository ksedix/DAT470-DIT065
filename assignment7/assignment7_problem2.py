...
def linear_scan(X, Q, b = None):
    ...
    #time taken to transfer data from numpy(CPU) to cupy(GPU)
    start_np_to_cp = time.time()
    X_cupy = cp.asarray(X, blocking=True)[cp.newaxis, :, :]
    Q_cupy = cp.asarray(Q, blocking=True)
    end_np_to_cp = time.time()

    for i in range(0,m,batch_size):
        batch_end = min(i + batch_size, m)
        batch = Q_cupy[i:batch_end]
        # Reshape Q and X for broadcasting
        # Q[:, np.newaxis, :] -> shape (m, 1, d)
        # X[np.newaxis, :, :] -> shape (1, n, d)
        diff = batch[:, cp.newaxis, :] - X_cupy

        # Calculate the Euclidean distance along the last axis (d dimension)
        # This results in a shape (m, n)
        D = cp.linalg.norm(diff, axis=2)

        #sort the values in the 2d array in descending order along the horizontal axis
        #return the index of the closest neighbor for each row
        I_batch = cp.argsort(D,axis=1)[:,0]

        #add all elements in I_batch to the I array
        I_cupy[i:batch_end] = I_batch

    # Synchronize to ensure all GPU operations are completed
    cp.cuda.Stream.null.synchronize()
    #time taken to transfer data from cupy(GPU) to numpy(GPU)
    start_cp_to_np = time.time()
    I = cp.asnumpy(I_cupy, blocking=True)
    end_cp_to_np = time.time()

    print("Time taken to transfer data from numpy to cupy:",end_np_to_cp-start_np_to_cp)
    print("Time taken to transfer data from cupy to numpy:",end_cp_to_np-start_cp_to_np)
